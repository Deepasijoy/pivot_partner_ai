...
import { Send, Briefcase, TrendingUp, FileText, Loader2 } from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ messages, onSendMessage, onQuickAction }) => {
  const [inputValue, setInputValue] = useState('');
  const [allMessages, setAllMessages] = useState<CopilotMessage[]>(messages);
  ...
  return (
    <div className="flex flex-col h-full bg-white border-r border-[#F5F5F5]">
      {/* Quick Action Pills - Enhanced */}
      <div className="p-4 border-b border-[#F5F5F5] bg-gradient-to-b from-white to-[#F5F5F5]">
        <p className="text-xs font-semibold text-[#333333]/50 mb-3 uppercase tracking-wider">Quick Actions</p>
          ...
          <div className="flex items-center justify-center h-full">
            <div className="text-center max-w-xs">
              <div className="w-12 h-12 bg-[#26c485]/10 rounded-full flex items-center justify-center mx-auto mb-3">
                <svg className="w-6 h-6 text-[#26c485]" fill="currentColor" viewBox="0 0 20 20">
                  <path d="M2 5a2 2 0 012-2h12a2 2 0 012 2v10a2 2 0 01-2 2H4a2 2 0 01-2-2V5z" />
                </svg>
              </div>
              <p className="text-sm text-[#333333]/60 font-medium">Start a conversation</p>
              <p className="text-xs text-[#333333]/40 mt-1">Click a quick action or ask me anything about remote careers</p>
            </div>
          </div>
        ) : (
        ...
        {isLoading && (
          <div className="flex justify-start animate-fade-in">
            <div className="bg-[#F5F5F5] rounded-lg rounded-bl-none px-4 py-3 flex items-center gap-2 border border-[#e0e0e0]">
              <div className="spinner w-4 h-4 border-2 border-[#26c485] border-t-transparent" />
              <span className="text-sm text-[#333333]/60">Groq is thinking...</span>
            </div>
          </div>
        )}
      ...
      <div className="border-t border-[#F5F5F5] p-4 bg-gradient-to-b from-white to-[#F5F5F5]">
        ...
        <p className="text-xs text-[#333333]/40 mt-2">Press Enter to send, Shift+Enter for new line</p>
      </div>
    </div>
  );
};
